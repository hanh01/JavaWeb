package com.example.asignment.config;

import com.example.asignment.service.UserService;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.antMatcher("/**").csrf().disable().authorizeRequests()
                .antMatchers("/", "/index","/chart").permitAll()
                .antMatchers("/css/**", "/js/**","/img/**","/scss/**","/vendor/**").permitAll()

//                admin,member in database role
                .antMatchers("/table","/add","edit").hasAuthority("admin")
